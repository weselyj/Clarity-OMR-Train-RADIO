...
**Recap.** This sub-project set out to test the Stage 3 v2 training audit's conclusion that the encoder DoRA adapters drifting during Stage 3 training (384/384 encoder LoRA params drifted; A2 encoder feature parity max_abs_diff 85.15) was the dominant cause of the demo-eval gap. The diagnostic in Phase 1 — a frankenstein checkpoint pairing the trained-against Stage 2 v2 encoder with the Stage 3 v2 decoder — gave mean onset_f1 of **0.0599** versus the Stage 3 v2 baseline of **0.0589**, a delta of **+0.001**. Gnossienne actually regressed by 0.025. This is well below the 0.10 gate threshold and nowhere near the ≥0.15 uplift that would have confirmed encoder drift as the dominant failure mode. The encoder-drift hypothesis is disproved; the v3 retrain was halted and two follow-up diagnostic streams launched.

**What Stream A revealed.** Running the Stage 3 v2 decoder against its own training labels on 20 stratified samples produced mean token accuracy **0.533** and exact-match rate **0.200** — both well below the 80% triage threshold for a decoder reproducing training data. The informative signal is the per-corpus split: synthetic_systems **0.113**, grandstaff_systems **0.400**, primus_systems **0.753**, cameraprimus_systems **0.865**. The decoder reproduces single-staff training labels (75–87% accuracy) but does not reproduce multi-staff piano training labels (11–40%). There is a known confound — Stream A runs the decoder against the live (drifted) encoder rather than the cached features it was trained on — but Phase 1 already established that swapping the encoder back to the trained-against S2v2 features gives only +0.001 mean onset_f1, so the per-corpus collapse on grand-staff content cannot be attributed to encoder mismatch.

**What Stream B revealed.** Across the four demo pieces, the raw decoder output (Stage 1) is already only **46–80%** of the reference note count — clair-de-lune 1306/1623 (80%), fugue 474/755 (63%), gnossienne 387/836 (46%), prelude 498/655 (76%). Pipeline stages 2 through 3 are token-identical to Stage 1 on three of four pieces; gnossienne loses 51 tokens (13%) to a staff-split validator failure on 2 systems, the only non-decoder pipeline drop worth noting. The Stage D diagnostic counters (`skipped_notes`, `skipped_chords`, `missing_durations`, `unknown_tokens`, `fallback_rests`) are all **zero** across all four pieces. The music21 → MusicXML conversion introduces small additional losses (5–145 notes) consistent with note-merging on dense ties, not with token suppression. The decoder undergenerates from the start.

**Dominant failure mode.** Streams A and B converge on a single diagnosis: the Stage 3 v2 decoder did not learn the multi-staff piano content it was trained on. The training-data side (Stream A) shows the decoder cannot reproduce its own grand-staff training labels (synthetic 0.113, grandstaff 0.400) while it handles single-staff training labels fine (primus 0.753, cameraprimus 0.865). The inference side (Stream B) shows the decoder undergenerates note tokens on the demo pieces — which are all grand-staff piano — and the assembly pipeline is nearly lossless on top of that undergeneration. The demo eval gap is therefore not an inference-pipeline problem and not an encoder-drift problem: it is a decoder that did not converge on multi-staff content during training. The encoder-freeze bug from the Stage 3 v2 audit is real and worth fixing for regression prevention, but fixing it alone will not close the demo gap.

**Recommendation for next sub-project.** Do not retrain Stage 3 with the freeze fix alone; that addresses a real bug but not the dominant failure mode. Two complementary follow-up investigations are warranted: **(a) training-data diagnosis** — compare per-corpus training-loss curves between single-staff and multi-staff corpora, inspect sequence length distributions, audit tokenization of grand-staff samples for encoding errors or truncation, and check whether the cached-feature training paradigm interacts badly with longer multi-staff sequences; this is cheap (no GPU training), bounded, and would directly inform a retrain recipe. **(b) Architecture/capacity question** — reproduce the upstream DaViT baseline on the same demo pieces to establish whether the current decoder has sufficient capacity for multi-staff piano, picking up the "out of scope" parallel thread from the original audit. Run (a) first because it is cheaper and may identify a fixable cause that obviates (b); keep (b) queued as the longer parallel thread if (a) does not surface a root cause.
