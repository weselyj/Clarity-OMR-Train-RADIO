...
Walks the scratch clone at C:\\Users\\Jonathan Wesely\\scratch\\omr-layout-analysis
and processes every .json annotation file found under a `json/` subdirectory.
For each annotation that has at least one staff, copies the matching image into
the target images/ dir and writes a single-class YOLO label into the target
labels/ dir, both keyed by a unique piece-prefixed filename.
...
SOURCE_ROOT = Path(r"C:\Users\Jonathan Wesely\scratch\omr-layout-analysis")
TARGET_LABELS = Path("data/processed/omr_layout_real/labels")
TARGET_IMAGES = Path("data/processed/omr_layout_real/images")
