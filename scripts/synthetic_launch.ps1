# Outer launcher: detaches the inner synthetic-render wrapper from SSH via WMI.
$ErrorActionPreference = "Stop"
$repo = Join-Path $env:USERPROFILE "Clarity-OMR-Train-RADIO"
$inner = Join-Path $repo "scripts\synthetic_inner.ps1"
...
$pidFile = Join-Path $repo "logs\synthetic_render.pid"
...
$logFile = Join-Path $repo "logs\synthetic_render.log"
$errFile = Join-Path $repo "logs\synthetic_render.err"
if (Test-Path $logFile) { Write-Output "--- log tail ---"; Get-Content $logFile -Tail 8 }
if ((Test-Path $errFile) -and (Get-Item $errFile).Length -gt 0) { Write-Output "--- err tail ---"; Get-Content $errFile -Tail 8 }
